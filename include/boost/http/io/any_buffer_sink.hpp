...
#ifndef BOOST_HTTP_IO_ANY_BUFFER_SINK_HPP
#define BOOST_HTTP_IO_ANY_BUFFER_SINK_HPP
...
public:
    ...
    */
    ...
    /** Write some data from a buffer sequence.
        ...
        Attempt to write up to `capy::buffer_size( buffers )` bytes from
...
private:
    ...
    */
    auto
    write_some_(std::span<capy::const_buffer const> buffers);
    ...
    */
    auto
    write_(std::span<capy::const_buffer const> buffers);
    ...
    */
    auto
    write_eof_buffers_(std::span<capy::const_buffer const> buffers);
};

/** Type-erased ops for awaitables that await-return `capy::io_result<>`. */
struct any_buffer_sink::awaitable_ops
{
    bool (*await_ready)(void*);
    std::coroutine_handle<> (*await_suspend)(void*, std::coroutine_handle<>, capy::io_env const*);
    capy::io_result<> (*await_resume)(void*);
...
{
    void (*destroy)(void*) noexcept;
    std::span<capy::mutable_buffer> (*do_prepare)(
        void* sink,
        std::span<capy::mutable_buffer> dest);
...
struct any_buffer_sink::vtable_for_impl
{
    ...
    static std::span<capy::mutable_buffer>
    do_prepare_impl(
        void* sink,
        std::span<capy::mutable_buffer> dest)
    {
...
inline std::span<capy::mutable_buffer>
any_buffer_sink::prepare(std::span<capy::mutable_buffer> dest)
{
    ...
    std::span<capy::const_buffer const> buffers)
{
    ...
    {
        any_buffer_sink* self_;
        std::span<capy::const_buffer const> buffers_;
...
any_buffer_sink::write_some(CB buffers)
{
    ...
    capy::mutable_buffer arr[capy::detail::max_iovec_];
...
any_buffer_sink::write_eof(CB buffers)
{
    ...
    // Synthesized path: prepare + capy::buffer_copy + commit + commit_eof
    capy::buffer_param<CB> bp(buffers);
    std::size_t total = 0;
