...
#ifndef BOOST_HTTP_IO_ANY_BUFFER_SOURCE_HPP
#define BOOST_HTTP_IO_ANY_BUFFER_SOURCE_HPP
...
#include <boost/http/concept/read_source.hpp>
...
/** Type-erased wrapper for any BufferSource.
    ...
    char buf[64];
    auto [ec2, n] = co_await abs.read_some(capy::mutable_buffer(buf, 64));
    @endcode
...
public:
    ...
    */
    ...
    /** Pull buffer data from the source.
        ...
        @return An awaitable that await-returns `(error_code,std::span<capy::const_buffer>)`.
            ...
            On EOF, `ec == capy::cond::eof` and span is empty.
    ...
    */
    ...
    /** Read some data into a mutable buffer sequence.
        ...
        Attempt to read up to `capy::buffer_size( buffers )` bytes into
        ...
        from @ref pull, @ref capy::buffer_copy, and @ref consume.
    ...
    */
    ...
    /** Read data into a mutable buffer sequence.
        ...
        synthesized from @ref pull, @ref capy::buffer_copy, and @ref consume.

        @param buffers The buffer sequence to fill.

        @return An awaitable that await-returns `(error_code,std::size_t)`.
            On success, `n == capy::buffer_size(buffers)`.
...
private:
    ...
    */
    auto
    read_some_(std::span<capy::mutable_buffer const> buffers);
    ...
    */
    auto
    read_(std::span<capy::mutable_buffer const> buffers);
};

/** Type-erased ops for awaitables that await-return `capy::io_result<std::span<capy::const_buffer>>`. */
struct any_buffer_source::awaitable_ops
{
    bool (*await_ready)(void*);
    std::coroutine_handle<> (*await_suspend)(void*, std::coroutine_handle<>, capy::io_env const*);
    capy::io_result<std::span<capy::const_buffer>> (*await_resume)(void*);
    void (*destroy)(void*) noexcept;
};

/** Type-erased ops for awaitables that await-return `capy::io_result<std::size_t>`. */
struct any_buffer_source::read_awaitable_ops
{
    bool (*await_ready)(void*);
    std::coroutine_handle<> (*await_suspend)(void*, std::coroutine_handle<>, capy::io_env const*);
    capy::io_result<std::size_t> (*await_resume)(void*);
    void (*destroy)(void*) noexcept;
};
...
struct any_buffer_source::vtable_for_impl
{
    using PullAwaitable = decltype(std::declval<S&>().pull(
        std::declval<std::span<capy::const_buffer>>()));
        ...
        std::span<capy::const_buffer> dest)
    {
        ...
        static constexpr awaitable_ops ops = {
            ...
            +[](void* p, std::coroutine_handle<> h, capy::io_env const* env) {
                return capy::detail::call_await_suspend(
        ...
        requires ReadSource<S>
    {
        using Aw = decltype(std::declval<S&>().read_some(
            std::span<capy::mutable_buffer const>{}));
        auto& s = *static_cast<S*>(source);
...
inline auto
any_buffer_source::pull(std::span<capy::const_buffer> dest)
{
    struct awaitable
    {
    ...
    std::span<capy::mutable_buffer const> buffers)
{
    ...
    {
        any_buffer_source* self_;
        std::span<capy::mutable_buffer const> buffers_;
