...
#ifndef BOOST_HTTP_IO_PULL_FROM_HPP
#define BOOST_HTTP_IO_PULL_FROM_HPP
...
{
    capy::mutable_buffer dst_arr[capy::detail::max_iovec_];
    std::size_t total = 0;
    ...
    for(;;)
    {
        ...
        auto [ec, n] = co_await source.read(
            std::span<capy::mutable_buffer const>(dst_bufs));
...
/** Transfer data from a capy::ReadStream to a BufferSink.
    ...
    @tparam Src The stream type, must satisfy @ref capy::ReadStream.
    @tparam Sink The sink type, must satisfy @ref BufferSink.
    ...
    @par Example
    @code
    task<void> transfer_body(capy::ReadStream auto& stream, BufferSink auto& sink)
    {
    ...
    @see capy::ReadStream, BufferSink, push_to
*/
template<capy::ReadStream Src, BufferSink Sink>
