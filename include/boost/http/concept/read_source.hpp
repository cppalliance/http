...
#ifndef BOOST_HTTP_CONCEPT_READ_SOURCE_HPP
#define BOOST_HTTP_CONCEPT_READ_SOURCE_HPP
...
/** Concept for types providing complete reads from a data source.
    ...
    A type satisfies `ReadSource` if it satisfies @ref capy::ReadStream
    and additionally provides a `read` member function that accepts
    any @ref MutableBufferSequence and await-returns
    `(error_code, std::size_t)`.

    `ReadSource` refines `capy::ReadStream`. Every `ReadSource` is a
    `capy::ReadStream`. Algorithms constrained on `capy::ReadStream` accept both
    ...
    @li `T` must satisfy @ref capy::ReadStream (provides `read_some`)
    @li `T` must provide a `read` member function template accepting
        any @ref MutableBufferSequence
    @li The return type of `read` must satisfy @ref capy::IoAwaitable
    ...
    `buffer_size( buffers )` bytes (partial read). See @ref capy::ReadStream.
    ...
    capy::IoAwaitable auto read_some( MB buffers );   // inherited from capy::ReadStream

    template< MutableBufferSequence MB >
    capy::IoAwaitable auto read( MB buffers );
    @endcode
    ...
    @see capy::ReadStream, capy::IoAwaitable, MutableBufferSequence,
        capy::awaitable_decomposes_to
*/
template<typename T>
concept ReadSource =
    capy::ReadStream<T> &&
    requires(T& source, capy::mutable_buffer_archetype buffers)
    {
        { source.read(buffers) } -> capy::IoAwaitable;
