...
private:
    template<class>
    friend class http::basic_router;
    ...
    BOOST_HTTP_DECL ~any_router();
    BOOST_HTTP_DECL any_router(opt_flags);
    BOOST_HTTP_DECL any_router(any_router&&) noexcept;
    BOOST_HTTP_DECL any_router(any_router const&) noexcept;
    BOOST_HTTP_DECL any_router& operator=(any_router&&) noexcept;
    BOOST_HTTP_DECL any_router& operator=(any_router const&) noexcept;
    BOOST_HTTP_DECL std::size_t count() const noexcept;
    BOOST_HTTP_DECL layer& new_layer(core::string_view pattern);
    BOOST_HTTP_DECL void add_impl(core::string_view, handler_list const&);
    BOOST_HTTP_DECL void add_impl(layer&,
        http::method, handler_list const&);
    BOOST_HTTP_DECL void add_impl(layer&,
        core::string_view, handler_list const&);
    BOOST_HTTP_DECL route_result resume_impl(
        route_params_base&, route_result ec) const;
    BOOST_HTTP_DECL route_result dispatch_impl(http::method,
        core::string_view, urls::url_view const&,
            route_params_base&) const;
    BOOST_HTTP_DECL route_result dispatch_impl(
        route_params_base&) const;
...
public:
    ...
    */
    ...
    /** Dispatch a request using a method string.
        ...
        that are not represented by @ref http::method.
